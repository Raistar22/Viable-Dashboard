...
function processAllClientsGmail() {
  ...
  function getProcessedMessageIds(bufferSheet) {
    try {
      ...
      if (messageIdIndex === -1) {
        warnLog('Message ID column not found in buffer sheet, cannot deduplicate properly');
        return processedIds;
      }
  ...
  function createBufferRowData(headers, data) {
    try {
      ...
      const columnMappings = {
        'Original File Name': data.originalFileName,
        'Changed File Name': data.changedFileName,
        ...
        'Email Sender': data.emailSender,
        'Date Added': data.dateAdded,
        'Last Modified': data.lastModified,
        'Processing Attempts': data.processingAttempts
      };
